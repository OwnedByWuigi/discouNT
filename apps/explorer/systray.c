...
static void add_taskbar_button( HWND hwnd )
{
    ...
    if (hwnd)
    {
        DWORD process, tray_process;
        if (!GetWindowThreadProcessId( hwnd, &process ) ||
            !GetWindowThreadProcessId( tray_window, &tray_process ) ||
            process == tray_process) return;
    }

    if (!(win = malloc( sizeof(*win) ))) return;
    win->hwnd = hwnd;
    win->button = CreateWindowW( WC_BUTTONW, hwnd ? NULL : start_label, WS_CHILD | BS_OWNERDRAW,
...
void initialize_systray( BOOL arg_using_root, BOOL arg_enable_shell, BOOL arg_show_systray, BOOL arg_no_tray_items )
{
    ...
    if (enable_taskbar)
    {
        /* discouNT is the display host.  There is no host window manager
           appbar to subtract, so anchor the shell tray deterministically. */
        screen_width = GetSystemMetrics( SM_CXSCREEN );
        screen_height = GetSystemMetrics( SM_CYSCREEN );

        tray_window = CreateWindowExW( WS_EX_NOACTIVATE, shell_traywnd_class.lpszClassName, NULL, WS_POPUP,
                                       0, screen_height - 28, screen_width, 28, 0, 0, 0, 0 );
    }
    else
    {
    ...
    if (!LoadStringW( NULL, IDS_START_LABEL, start_label, ARRAY_SIZE( start_label )))
        lstrcpyW( start_label, L"Start" );
