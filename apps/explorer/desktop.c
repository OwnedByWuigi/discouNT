    ...
    case WM_PAINT:
        {
            ...
            BeginPaint( hwnd, &ps );
            PaintDesktop( ps.hdc );
            if (!using_root) draw_launchers( ps.hdc, ps.rcPaint );
            EndPaint( hwnd, &ps );
        }
        return 0;
    }
...
void manage_desktop( WCHAR *arg )
{
    ...
    /* Wine root mode relies on its host display driver to supply the root
       surface.  discouNT is the display host, so use Explorer's own desktop,
       launcher and taskbar paths when no named virtual desktop was given. */
    if (!name)
    {
        using_root = FALSE;
        enable_shell = TRUE;
