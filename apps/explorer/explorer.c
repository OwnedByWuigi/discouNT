            ...
            else
            {
                ...
                else if(ILIsParent(full_pidl,info->pidl,FALSE))
                {
                    ...
                    while(!ILIsEmpty(next_pidl))
                    {
                        ...
                        ILFree(first);
                        next_pidl = (LPITEMIDLIST)ILGetNext(next_pidl);
                    }
...
static LRESULT CALLBACK explorer_wnd_proc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    /* USER32 delivers creation-time messages synchronously.  The per-window
       state is installed immediately after CreateWindowW returns, so early
       WM_SIZE/WM_NOTIFY messages must use default handling. */
    if (!info && uMsg != WM_QUIT)
