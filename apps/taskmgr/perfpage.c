...
{
        static int logged_first_wake = 0;
	...
	hPerformancePageEvent = CreateEventW(NULL, TRUE, TRUE, NULL);
        SerialPutString("[TASKMGR] PerformancePageRefreshThread start\r\n");
	...
	while (1)
	{
		...
		{
                        if (!logged_first_wake) {
                                SerialPutString("[TASKMGR] PerformancePageRefreshThread first wake\r\n");
                                logged_first_wake = 1;
                        }
			ULONG CpuUsage;
