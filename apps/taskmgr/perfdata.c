...
BOOL PerfDataInitialize(void)
{
    ...
    SerialPutString("[TASKMGR] PerfDataInitialize enter\r\n");
    InitializeCriticalSection(&PerfDataCriticalSection);
    SerialPutString("[TASKMGR] PerfDataInitialize cs init ok\r\n");

    /*
     * Get number of processors in the system
     */
    status = NtQuerySystemInformation(SystemBasicInformation, &SystemBasicInfo, sizeof(SystemBasicInfo), NULL);
    SerialPutString("[TASKMGR] PerfDataInitialize NtQuerySystemInformation returned\r\n");
    if (status != NO_ERROR)
        return FALSE;
    
    SerialPutString("[TASKMGR] PerfDataInitialize leave ok\r\n");
    return TRUE;
}
