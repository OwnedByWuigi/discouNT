...
static BOOL OnCreate(HWND hWnd)
{
    ...
    SerialPutString("[TASKMGR] OnCreate begin\r\n");
    ...
    if(!hStatusWnd)
        return FALSE;
    SerialPutString("[TASKMGR] OnCreate status bar ok\r\n");
    ...
    SerialPutString("[TASKMGR] OnCreate child pages ok\r\n");
#else
    ...
    SerialPutString("[TASKMGR] OnCreate timer set\r\n");
    ...
    PerfDataRefresh();
    PerfDataRefresh();
    SerialPutString("[TASKMGR] OnCreate initial refresh ok\r\n");
                     ...
                     int       nCmdShow)
{
    ...
    SerialPutString("[TASKMGR] WinMain start\r\n");
    InitCommonControls();
    SerialPutString("[TASKMGR] InitCommonControls ok\r\n");
    ...
    SerialPutString("[TASKMGR] OpenProcess ok\r\n");
    SetPriorityClass(hProcess, HIGH_PRIORITY_CLASS);
    CloseHandle(hProcess);
    SerialPutString("[TASKMGR] priority setup ok\r\n");
    ...
    if (OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &hToken)) {
        SerialPutString("[TASKMGR] OpenProcessToken ok\r\n");

        /* Get the LUID for the debug privilege.  */
        LookupPrivilegeValueW(NULL, L"SeDebugPrivilege", &tkp.Privileges[0].Luid);
        SerialPutString("[TASKMGR] LookupPrivilegeValueW ok\r\n");
        ...
        SerialPutString("[TASKMGR] AdjustTokenPrivileges ok\r\n");
    }

    /* Load our settings from the registry */
    LoadSettings();
    SerialPutString("[TASKMGR] LoadSettings ok\r\n");

    /* Initialize perf data */
    if (!PerfDataInitialize()) {
        SerialPutString("[TASKMGR] PerfDataInitialize failed\r\n");
        return -1;
    }
    SerialPutString("[TASKMGR] PerfDataInitialize ok\r\n");

    DialogBoxW(hInst, (LPWSTR)IDD_TASKMGR_DIALOG, NULL, TaskManagerWndProc);
    SerialPutString("[TASKMGR] DialogBoxW returned\r\n");
 
    /* Save our settings to the registry */
    SaveSettings();
    SerialPutString("[TASKMGR] SaveSettings ok\r\n");
    return 0;
}
