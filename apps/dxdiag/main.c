...
static void append_information_line(WCHAR *text, const WCHAR *name, const WCHAR *value)
{
    size_t length = 0;
    const WCHAR *source;

    if (!text || !name) return;
    while (text[length] && length < 2047) length++;

    source = name;
    while (*source && length < 2047) text[length++] = *source++;
    if (length < 2047) text[length++] = L':';
    if (length < 2047) text[length++] = L' ';

    source = value ? value : L"(none)";
    while (*source && length < 2047) text[length++] = *source++;
    if (length < 2047) text[length++] = L'\r';
    if (length < 2047) text[length++] = L'\n';
    text[length] = 0;
}

static void show_information_dialog(const struct dxdiag_information *dxdiag_info)
{
    const struct system_information *info = &dxdiag_info->system_info;
    WCHAR title[MAX_STRING_LEN] = L"DirectX Diagnostic Tool";
    WCHAR text[2048];

    text[0] = 0;
    append_information_line(text, L"Time", info->szTimeEnglish);
    append_information_line(text, L"Machine name", info->szMachineNameEnglish);
    append_information_line(text, L"Operating system", info->szOSExLongEnglish);
    append_information_line(text, L"Language", info->szLanguagesEnglish);
    append_information_line(text, L"System manufacturer", info->szSystemManufacturerEnglish);
    append_information_line(text, L"System model", info->szSystemModelEnglish);
    append_information_line(text, L"BIOS", info->szBIOSEnglish);
    append_information_line(text, L"Processor", info->szProcessorEnglish);
    append_information_line(text, L"Memory", info->szPhysicalMemoryEnglish);
    append_information_line(text, L"Page file", info->szPageFileEnglish);
    append_information_line(text, L"Windows directory", info->szWindowsDir);
    append_information_line(text, L"DirectX version", info->szDirectXVersionLongEnglish);
    append_information_line(text, L"DxDiag version", info->szDxDiagVersion);
    LoadStringW(hInstance, STRING_DXDIAG_TOOL, title, ARRAY_SIZE(title));
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPWSTR cmdline, int cmdshow)
{
    ...
    if (info.output_type != OUTPUT_NONE)
        ...
        show_information_dialog(dxdiag_info);
