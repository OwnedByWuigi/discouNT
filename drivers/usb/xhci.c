...
static int configure_mass_storage(XHCI_DEVICE *device, uint8_t *config,
    ...
    if (!in || !out || !ring_allocate(&device->in_ring) || !ring_allocate(&device->out_ring)) return 0;
    ...
    device->configured = 1;
    return UsbMscAttach(device, bulk_transfer, in, out) != 0;
}
...
static int enumerate_port(XHCI_STATE *state, uint8_t port) {
    ...
    if (!wait_bits(port_reg, 0, XHCI_PORT_RESET, 0) || !wait_bits(port_reg, 0, XHCI_PORT_PED, 1)) return 0;
    ...
    if (!command(state, 0, XHCI_TRB_TYPE(9), &device->slot_id) || !device->slot_id) return 0;
    ...
    if (!command(state, physical(device->input_context), XHCI_TRB_TYPE(11) |
                 ((uint32_t)device->slot_id << 24), 0)) return 0;
