...
static uintptr_t mmio_bar(USB_CONTROLLER*c){uint32_t l=UsbPciRead(c,0x10);uintptr_t a=l&0xFFFFFFF0U;
#if UINTPTR_MAX > 0xFFFFFFFFU
 if((l&6)==4)a|=(uintptr_t)UsbPciRead(c,0x14)<<32;
#endif
 return a;}
...
void UsbInit(void){controller_count=0;
 for(uint16_t b=0;b<256&&controller_count<MAX_CONTROLLERS;b++)for(uint8_t s=0;s<32&&controller_count<MAX_CONTROLLERS;s++)for(uint8_t f=0;f<8&&controller_count<MAX_CONTROLLERS;f++){
  ...
  if(c->type!=USB_HC_UHCI)c->base=mmio_bar(c);UsbPciWrite(c,4,UsbPciRead(c,4)|6);SerialPutString("[USB] Found ");SerialPutString(type_name(c->type));SerialPutString(" controller\r\n");if(start(c)){c->initialized=1;controller_count++;}
