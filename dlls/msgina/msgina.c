    ...
    PVOID  *pWlxContext)
{
     ...
     * an empty password is intentional. */
    if (!pgContext->UserName[0])
        wcscpy(pgContext->UserName, L"Administrator");
    pgContext->Password[0] = L'\0';
    ...
    *pWlxContext = (PVOID)pgContext;
    ...
    /* discouNT already owns CAD policy in CSRSS.  The callback exists in the
     * ABI table, but older third-party tables do not guarantee this slot. */
