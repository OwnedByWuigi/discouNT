...
GinaBannerWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (msg == WM_PAINT) {
        static HBITMAP logo;
        if (!logo)
        logo = (HBITMAP)LoadImageW(hDllInstance,
...
{
    WNDCLASSW banner_class;
    static const DISCOUNT_DIALOG_CONTROL logon_controls[] = {
        {L"MSGINA_BANNER", L"", WS_CHILD | WS_VISIBLE,
            0, IDC_ROSLOGO, 8, 0, 413, 72},
        {L"Static", L"Username:", WS_CHILD | WS_VISIBLE, 0, (UINT)-1, 20, 82, 52, 8},
        {L"Edit", L"", WS_CHILD | WS_VISIBLE | WS_TABSTOP | ES_AUTOHSCROLL,
            0, IDC_LOGON_USERNAME, 80, 79, 220, 14},
        {L"Static", L"Password:", WS_CHILD | WS_VISIBLE, 0, (UINT)-1, 20, 100, 52, 8},
        {L"Edit", L"", WS_CHILD | WS_VISIBLE | WS_TABSTOP | ES_AUTOHSCROLL | ES_PASSWORD,
            0, IDC_LOGON_PASSWORD, 80, 98, 220, 14},
        {L"Static", L"Log on to:", WS_CHILD | WS_VISIBLE, 0, (UINT)-1, 20, 118, 52, 8},
        {L"ComboBox", L"", WS_CHILD | WS_VISIBLE | WS_TABSTOP | CBS_DROPDOWNLIST | WS_VSCROLL,
            0, IDC_LOGON_DOMAIN, 80, 117, 220, 14},
        {L"Button", L"OK", WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_DEFPUSHBUTTON,
            0, IDOK, 180, 148, 60, 16},
        {L"Button", L"Cancel", WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_PUSHBUTTON,
            0, IDCANCEL, 245, 148, 60, 16},
        {L"Button", L"&Shut Down", WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_PUSHBUTTON,
            0, IDC_LOGON_SHUTDOWN, 310, 148, 75, 16},
    };
    static const DISCOUNT_DIALOG_TEMPLATE logon_template = {
        IDD_LOGON, L"Logon", WS_POPUP | WS_CAPTION | WS_SYSMENU |
            WS_BORDER | WS_DLGFRAME | WS_VISIBLE,
        430, 190, logon_controls,
        (int)(sizeof(logon_controls) / sizeof(logon_controls[0]))
    };
    TRACE("GUIInitialize(%p)\n", pgContext);
    ZeroMemory(&banner_class, sizeof(banner_class));
    banner_class.lpfnWndProc = GinaBannerWndProc;
    banner_class.lpszClassName = L"MSGINA_BANNER";
    RegisterClassW(&banner_class);
    User32RegisterDialogTemplate(&logon_template);
    return TRUE;
}

static VOID
GinaPaintLogonBanner(HDC hdc)
{
    RECT banner;
    HBRUSH blue;
    HBRUSH light;
    ...
    if (blue) {
        FillRect(hdc, &banner, blue);
        DeleteObject(blue);
    }
    TextOutW(hdc, 12, 18, L"discouNT", 8);

    light = CreateSolidBrush(RGB(192, 192, 192));
    if (light) {
        banner.top = 71;
        banner.bottom = 72;
        FillRect(hdc, &banner, light);
        DeleteObject(light);
    }
}
    ...
    IN LPARAM lParam)
{
    ...
    switch (uMsg)
    {
        case WM_INITDIALOG:
        {
            ...
            CreateWindowExW(0, L"MSGINA_BANNER", L"",
                            WS_CHILD | WS_VISIBLE,
                            0, 0, 275, 54, hwndDlg,
                            (HMENU)(UINT_PTR)IDC_ROSLOGO,
                            pDlgData->pgContext->hDllInstance, NULL);
        ...
        case WM_PAINT:
        {
            ...
            if (pDlgData)
            {
                ...
                /* MSGINA owns and paints its own logon artwork. */
                GinaPaintLogonBanner(ps.hdc);
                EndPaint(hwndDlg, &ps);
            }
            return TRUE;
        }
    ...
    IN OUT PGINA_CONTEXT pgContext)
{
    ...
    SerialPutString("[MSGINA] DoLoginTasks returned\r\n");
    ...
    {
        SerialPutString("[MSGINA] CreateProfile failed\r\n");
    ...
    result = TRUE;
    SerialPutString("[MSGINA] DoLogon success\r\n");
    ...
    IN LPARAM lParam)
{
    ...
    switch (uMsg)
    {
        ...
        case WM_COMMAND:
            SerialPutString("[MSGINA] LogonDialogProc WM_COMMAND\r\n");
            switch (LOWORD(wParam))
            {
                case IDOK:
                    SerialPutString("[MSGINA] IDOK\r\n");
