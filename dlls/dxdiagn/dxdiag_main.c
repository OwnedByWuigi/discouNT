...
static HRESULT WINAPI DXDiagCF_CreateInstance(IClassFactory *iface, IUnknown *pOuter, REFIID riid,
        void **ppv)
{
  SerialPutString("[DXDIAGN] CreateInstance enter\r\n");
  TRACE("(%p)->(%p,%s,%p)\n", iface, pOuter, debugstr_guid(riid), ppv);
  {
    HRESULT hr = DXDiag_CreateDXDiagProvider(iface, pOuter, riid, ppv);
    SerialPutString("[DXDIAGN] CreateInstance leave\r\n");
    return hr;
  }
}
...
{
    SerialPutString("[DXDIAGN] DllGetClassObject enter\r\n");
    TRACE("(%s,%s,%p)\n", debugstr_guid(rclsid), debugstr_guid(riid), ppv);

    if (IsEqualGUID(rclsid, &CLSID_DxDiagProvider)) {
      *ppv = &DXDiag_CF.IClassFactory_iface;
      SerialPutString("[DXDIAGN] DllGetClassObject leave\r\n");
      return S_OK;
    }
