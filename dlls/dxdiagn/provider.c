        ...
        DXDIAG_INIT_PARAMS *pParams)
{
    ...
    SerialPutString("[DXDIAGN] Initialize enter\r\n");
    TRACE("(%p,%p)\n", iface, pParams);
    ...
    if (!This->info_root)
    {
        ...
        SerialPutString("[DXDIAGN] information tree returned\r\n");
        SerialPutString("[DXDIAGN] information tree hr ");
        SerialPrintDec((uint32_t)hr);
        SerialPutString("\r\n");
        if (FAILED(hr)) {
            /* Keep initialization usable if a nonessential provider branch
             * failed after constructing the system information tree. */
            if (!This->info_root) return hr;
        }
    }

    This->init = TRUE;
    memcpy(&This->params, pParams, pParams->dwSize);
    SerialPutString("[DXDIAGN] Initialize leave\r\n");
    return S_OK;
}
        ...
        IDxDiagContainer **ppInstance)
{
  ...
  SerialPutString("[DXDIAGN] GetRootContainer enter\r\n");
  TRACE("(%p,%p)\n", iface, ppInstance);
  ...
  if (FALSE == This->init) {
    ...
    {
    HRESULT hr = DXDiag_CreateDXDiagContainer(&IID_IDxDiagContainer, This->info_root,
            &This->IDxDiagProvider_iface, (void **)ppInstance);
    SerialPutString("[DXDIAGN] GetRootContainer leave\r\n");
    SerialPutString("[DXDIAGN] GetRootContainer result ");
    SerialPrintDec((uint32_t)hr);
    SerialPutString(" object ");
    SerialPrintHex((uint32_t)(ppInstance ? *ppInstance : NULL));
    SerialPutString("\r\n");
    return hr;
  }
}
...
static HRESULT build_systeminfo_tree(IDxDiagContainerImpl_Container *node)
{
    ...
    swprintf(print_buf, ARRAY_SIZE(print_buf), L"%uMB RAM",
...
static HRESULT build_information_tree(IDxDiagContainerImpl_Container **pinfo_root)
{
    ...
    static const char *root_debug_names[] =
    {
        "SystemInfo", "DisplayDevices", "DirectSound", "DirectMusic", "DirectInput",
        "DirectPlay", "SystemDevices", "DirectXFiles", "DirectShowFilters", "LogicalDisks"
    };
    ...
    for (index = 0; index < ARRAY_SIZE(root_children); index++)
    {
        ...
        SerialPutString("[DXDIAGN] subsystem enter ");
        SerialPutString(root_debug_names[index]);
        SerialPutString("\r\n");
        ...
        SerialPutString("[DXDIAGN] subsystem leave ");
        SerialPutString(root_debug_names[index]);
        SerialPutString("\r\n");
        if (FAILED(hr))
        {
            /* Individual DirectX providers are optional in this runtime.
             * Preserve the node and continue building the usable portions of
             * the DxDiag tree when one provider is not registered. */
